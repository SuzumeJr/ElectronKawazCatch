...
{
    if (this.state_ === MainSceneLayer.GameState["PLAYING"])
    {
        // プレイ中のとき
...
/**
 * シーンレイヤー内で使用するフルーツの処理
 */
function SceneLayeraddFruit()
{
    // 画面サイズを取り出す
    var winSize = cc.director.getWinSize();
...
function SceneLayerRemoveFruit(fruit)
{
    var removed = false;
...
function SceneLayerCatchFruit(fruit)
{
...
function SceneLayerAddReadyLabel()
{
    var winSize = cc.director.getWinSize();
 ...
 * メインシーンレイヤークラスの構築
 */
function CreateClassMainSceneLayer()
{
    ...
    MainSceneLayer = cc.Layer.extend({
        ...
        addFruit: SceneLayeraddFruit,
        removeFruit: SceneLayerRemoveFruit,
        catchFruit: SceneLayerCatchFruit,
        onCatchBomb: SceneLayerCatchBomb,

        addReadyLabel: SceneLayerAddReadyLabel,
        onResult: SceneLayerResult,
    });    
...
var MainSceneLayer = CreateClassMainSceneLayer();
